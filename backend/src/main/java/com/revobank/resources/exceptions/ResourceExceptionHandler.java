package com.revobank.resources.exceptions;
...
public class ResourceExceptionHandler {
	...
	@ExceptionHandler(ForbiddenArgumentException.class)
	public ResponseEntity<StandardError> forbiddenArgument(ForbiddenArgumentException e, HttpServletRequest request){
		HttpStatus status = HttpStatus.FORBIDDEN;
		StandardError err = new StandardError();
		err.setTimestamp(Instant.now());
		err.setStatus(status.value());
		err.setError("Forbidden argument");
		err.setMessage(e.getMessage());
		err.setPath(request.getRequestURI());
		return ResponseEntity.status(status).body(err);
	}
